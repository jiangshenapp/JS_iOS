...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary<NSString *,NSString *> *)attributeDict {
    if ([elementName isEqualToString:@"province"]) {
        ...
        CustomCityModel *model = [[CustomCityModel alloc] initWithDictionary:attributeDict];
...
- (NSString *)searchByKey:(NSString *)key valueName:(NSString *)valueName type:(NSString *)type {
    ...
    if (addressModel) {
        ...
        CustomCityModel *cityModel = (CustomCityModel *)[self searchModelInArr:addressModel.list key:arr[1] type:type];
        if (cityModel) {
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {
        ...
        CustomCityModel *cityModel = addressModel.list[row];
        return cityModel.name;
    } else if (component == 2) {
        AddressModel *addressModel = self.dataArr[self.selectedIndex_province];
        CustomCityModel *cityModel = addressModel.list[self.selectedIndex_city];
