...
- (void)awakeFromNib {
    [super awakeFromNib];
    self.starSpacing = 3;
    self.starSize = CGSizeMake(self.height, self.height);
    [self setUpSubviews];
}
...
- (void)setUpSubviews {
    normalImgName = @"app_score_nochoice";
    selectImgName = @"app_score_choice";
    if (self.starCount <= 0) {
        _starCount = 5;
    }
...
- (void)setStarImage {
    ...
    if (!self.starBrightImageName) {
        self.starBrightImageName = selectImgName;
    }

    if (!self.starDarkImageName) {
        self.starDarkImageName = normalImgName;
    }
    ...
    if (starBrightImage == nil) {
        starBrightImage = [UIImage imageNamed:selectImgName inBundle:resourcesBundle compatibleWithTraitCollection:nil];
        starBrightHighlightedImage = nil;
    }
    
    if (starDarkImage == nil) {
        starDarkImage = [UIImage imageNamed:normalImgName inBundle:resourcesBundle compatibleWithTraitCollection:nil];
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        ...
        if (i < self.starScore) {
            [starBtn setImage:starBrightImage forState:UIControlStateNormal];
            [starBtn setImage:starBrightHighlightedImage forState:UIControlStateHighlighted];
        } else {
            [starBtn setImage:starDarkImage forState:UIControlStateNormal];
            [starBtn setImage:starDarkHighlightedImage forState:UIControlStateHighlighted];
        }
    }
}
