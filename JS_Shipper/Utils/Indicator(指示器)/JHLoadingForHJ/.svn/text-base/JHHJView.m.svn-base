...
+ (JHHJView *)shareInstanceJHHJView{
    ...
    if (!instance) {
        ...
        instance.centerBGView.center = CGPointMake(WIDTH/2, instance.height/2);
...
+ (void)showLoadingOnView:(UIView *)superView Type:(JHHJViewType)type{
    ...
    switch (type) {
        case JHHJViewTypeSingleLine:
        {
            ...
            layer.position = CGPointMake(CGRectGetWidth(instance.centerBGView.frame)/2 - 17.5, CGRectGetHeight(instance.centerBGView.frame)/2);
...
- (CALayer *)lineAnimation{
    ...
    shape.frame                   = CGRectMake(0, 0, 12, 12);
    shape.path                    = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(0, 0, 12, 12)].CGPath;
    shape.fillColor               = LightYellowColor.CGColor;
    [shape addAnimation:[JHHJAnimation animationForScaleSmall] forKey:nil];

    
    /*        创建克隆层的持有对象         */
    CAReplicatorLayer *replicator = [CAReplicatorLayer layer];
    replicator.frame              = CGRectMake(0, 0, 12, 12);
    replicator.instanceDelay      = 0.35;
    replicator.instanceCount      = 3;
    replicator.instanceTransform  = CATransform3DMakeTranslation(18, 0, 0);
