...
#define ItemSelectedImgW [[UIScreen mainScreen] bounds].size.width / 2
#define ItemSelectedImgH 2
...
- (void)updateFrameOfSelectedBgWithIndex:(NSInteger)index {
    ...
    YPTabItem *item = self.items[index];
    self.itemSelectedBgImageView.frame = CGRectMake(item.frameWithOutTransform.size.width/2-ItemSelectedImgW/2,
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.itemSelectedBgScrollFollowContent) {
        ...
        frame.origin.x = rightScale * xDiff + leftItem.frameWithOutTransform.origin.x + self.itemSelectedBgInsets.left + (leftItem.frameWithOutTransform.size.width - ItemSelectedImgW)/2.0;
        ...
        if (widthDiff != 0) {
            ...
            frame.size.width = ItemSelectedImgW;
        }
